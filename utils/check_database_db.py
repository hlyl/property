...
with Session(engine) as session:
    ...
    df_sold = pd.read_sql("SELECT sold, COUNT(*) as count FROM property GROUP BY sold", con=session.connection())
    print("\nBy sold status:")
    print(df_sold.to_string(index=False))

    # Check review status for unsold properties
    df_review = pd.read_sql("SELECT review_status, COUNT(*) as count FROM property WHERE sold = 0 GROUP BY review_status", con=session.connection())
